...
2. **Check the tree for junk before it reaches `main`.** A broad `git add -A` picks up
   ...
   way, from an unquoted `>=10` in an interactive shell.
   ```bash
   git ls-files | while read -r f; do [ -s "$f" ] || echo "EMPTY: $f"; done
   git ls-files | grep -vE '^[A-Za-z0-9_./-]+$' | sed 's/^/ODD NAME: /'
   ```
   Both should print nothing. An empty tracked file is almost always an accident; a path
   with characters outside `[A-Za-z0-9_./-]` is almost always a redirect or a quoting bug.
...
4. **CHANGELOG entry at the top**, matching the existing format exactly — a bare `M/D/YYYY`
...
7. **Watch the build and verify every asset.** Five are expected: linux-gnu, linux-gnu-rhel8,
